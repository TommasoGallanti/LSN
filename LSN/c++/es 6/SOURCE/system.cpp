...
void System :: Verlet(){
  ...
  _naccepted += _npart;
  //cout <<"forze(x,y,z)" << _fx(_npart-1) << _fy(_npart-1) << _fz(_npart-1) << endl;
  //cout <<"pos finale(x)  "<< _particle(_npart-1).getposition(0,true) << endl;
  return;
  
}

double System :: Force(int i, int dim){   //mi chiedo quali sono le forze sulla part i-esima
  double f=0.0, dr;
  vec distance;
...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    // Compute the local magnetization excluding spin i
    // sum_neighbors = 0;
   // sum_neighbors = _particle((i+1)%_npart).getspin()+_particle((i-1+_npart)%_npart).getspin() + _H;

    // Compute the conditional probability P(s_i | s_{neighbors})
    double prob_up = exp(-2.0 * _J * _beta * (_particle(this->pbc(i+1)).getspin() + _particle(this->pbc(i-1)).getspin() + _H));
    double p_up = 1./(1 + prob_up);
    // Generate a random number
    double rand_num = _rnd.Rannyu();

    // Decide whether to flip the spin or not based on the conditional probability
    if(rand_num <  p_up) {
        // Spin i is up
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      for(int i=0; i<_npart; i++){ 
        ...
        //cout << _particle(i).getposition(0,true) << endl;
        //cout << _particle(i).getposition(1,true) << endl;
        //cout << _particle(i).getposition(2,true) << endl;
      }
    ...
    } else if( property == "N_EQ_BLOCKS" ){
      input >> _n_eq_blocks;
      coutf << "N_EQ_BLOCKS= " << _n_eq_blocks << endl;
    } else if( property == "N_EQ_STEPS" ){
      input >> _n_eq_steps;
      coutf << "N_EQ_STEPS= " << _n_eq_steps << endl;
    } else if( property == "TEMP_RESTARTED" ){
      input >> _temp_restarted;
      coutf << "TEMP_RESTARTED= " << _temp_restarted << endl;
    } else if( property == "J" ){
      input >> _J;
      coutf << "J= " << _J << endl;
    } else if( property == "H" ){
  ...
  input.close();
  //this->read_configuration(); .xyz
  this->read_configuration_ising();  //config.ising
...
void System :: initialize_for_iteration(double Temp){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  ifstream input("../INPUT/input.dat"); // Start reading ../INPUT/input.dat
  ofstream coutf;
  coutf.open("../OUTPUT/output(T).dat");
  string property;
  double delta;
  while ( !input.eof() ){
    input >> property;
    if( property == "SIMULATION_TYPE" ){
      ...
      input >> _H;
      coutf << "H= " << _H << endl;
    }else if( property == "H_MAGN" ){
      input >> _H_MAGN;
      coutf << "H_magn= " << _H_MAGN << endl;
    } else if( property == "E_TOT" ){
      input >> _H;
      coutf << "E_TOT= " << _H << endl;
    } else if( property == "ENDINPUT" ){
      coutf << "Reading input completed!" << endl;
      cerr << "readind input completed" << endl;
      break;
    } else cerr << "PROBLEM: unknown inputttt" << property << endl;
  }
  input.close();
  //this->read_configuration(); .xyz
  this->read_configuration_ising();  //config.ising
  ...
  if(_restart and _sim_type==0){
    ...
    if(cinf.is_open()){
      ...
      for(int i=0; i<_npart; i++){
        ...
        //cout<< "velocità inizializzate" << endl;
      }
    } else cerr << "PROBLEM: Unable to open INPUT file velocities.in"<< endl;
    cinf.close();
  } else {
    cout << "initialize velocities" << endl;
    cout << _temp << endl;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp("../OUTPUT/potential_energy(T).dat");
        coutp << "#     BLOCK:  ACTUAL_PE:     PE_AVE:      ERROR:        T:" << endl;
        coutp.close();
        _nprop++;
        ...
        ofstream coutk("../OUTPUT/kinetic_energy(T).dat");
        coutk << "#     BLOCK:   ACTUAL_KE:    KE_AVE:      ERRORV        T::" << endl;
        coutk.close();
        _nprop++;
        ...
        ofstream coutk("../OUTPUT/total_energy(T).dat");
        coutk << "#     BLOCK:   ACTUAL_TE:    TE_AVE:      ERROR:        T::" << endl;
        coutk.close();
        _nprop++;
        ...
        ofstream coutpr("../OUTPUT/pressure(T).dat");
        coutpr << "#     BLOCK:   ACTUAL_P:     P_AVE:       ERROR:        T::" << endl;
        coutpr.close();
        _nprop++;
        ...
        ofstream coutgr("../OUTPUT/gofr(T).dat");
        coutgr << "# DISTANCE:     AVE_GOFR:        ERROR:        T:" << endl;
        coutgr.close();
        input>>_n_bins;
        _nprop+=_n_bins;
        ...
        ofstream coutpr("../OUTPUT/magnetization(T).dat");
        coutpr << "#     BLOCK:   ACTUAL_M:     M_AVE:       ERROR:        T:" << endl;
        coutpr.close();
        _nprop++;
        ...
        ofstream coutpr("../OUTPUT/specific_heat(T).dat");
        coutpr << "#     BLOCK:   ACTUAL_CV:    CV_AVE:      ERROR:        T:" << endl;
        coutpr.close();
        _nprop++;
        _measure_cv = true;
        ...
        ofstream coutpr("../OUTPUT/susceptibility(T).dat");
        coutpr << "#     BLOCK:   ACTUAL_X:     X_AVE:       ERROR:        T:" << endl;
        coutpr.close();
        _nprop++;
        _measure_chi = true;
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        ofstream coutt("../OUTPUT/total_energy.dat" + to_string(_temp));
        ...
        ofstream coutteq("../OUTPUT/temperature_eq.dat");
        coutte << "#     BLOCK:   ACTUAL_T:     T_AVE:       ERROR:" << endl;
        coutteq << "#     BLOCK:   ACTUAL_T:     T_AVE:       ERROR:" << endl;
        coutte.close();
        coutteq.close();
        _nprop++;
        _measure_temp = true;
        ...
        ofstream coutpr("../OUTPUT/magnetization.dat"+ to_string(_temp));
        ...
        ofstream coutpr("../OUTPUT/specific_heat.dat"+ to_string(_temp));
        ...
        _measure_cv = true;
        _index_cv = index_property;
        //_index_tenergy2 = index_property;
        //index_property++;
        index_property++;
      } else if( property == "SUSCEPTIBILITY" ){
        ofstream coutpr("../OUTPUT/susceptibility.dat"+ to_string(_temp));
  ...
  _global_av.zeros();
  _global_av2.zeros();
  _average_eq.resize(_nprop);
  _block_av_eq.resize(_nprop);
  _global_av_eq.resize(_nprop);
  _global_av2_eq.resize(_nprop);
  _average_eq.zeros();
  _global_av_eq.zeros();
  _global_av2_eq.zeros();
  _nattempts = 0;
  _naccepted = 0;
  return;
}
...
// Read configuration from a .ising file in directory ../OUTPUT/CONFIG/
void System :: read_configuration_ising(){
  ifstream cinf;
  cinf.open("../INPUT/CONFIG/config.ising");
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
    ...
    cinf >> ncoord;
    if (ncoord != _npart){
      cerr << "PROBLEM: conflicting number of coordinates in input.dat & config.ising not match!" << endl;
      exit(EXIT_FAILURE);
    }
    cinf >> comment;
    for(int i=0; i<_npart; i++){
...
void System :: measure(){ // Measure properties
  ...
  double prex_temp=0.0; // temporary accumulator for prex
  double tenergy_temp=0.0;
  double tenergy_temp_squared = 0.0;
    ...
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(dr < _r_cut){
          ...
          if(_measure_pressure) prex_temp    += (1/pow(dr,12) - 1/(2*pow(dr,6))); // PRESSURE ... TO BE FIXED IN EXERCISE 4 
        }
      }
    }
  }
  ...
  if (_measure_tenergy){
    if (_sim_type < 2) _measurement(_index_tenergy) = kenergy_temp + penergy_temp;
      ...
      for (int i=0; i<_npart; i++){
        ...
        tenergy_temp_squared += pow(- _J * s_i * s_j - 0.5 * _H * (s_i + s_j),2);
      }
      tenergy_temp /= double(_npart);
      //tenergy_temp_squared /= double(_npart);
      _measurement(_index_tenergy) = tenergy_temp;
      //_measurement(_index_tenergy2) = tenergy_temp_squared;
    }
  ...
  if (_measure_pressure){
     prex_temp = _rho*_temp + 1.0/(3.0*_volume)*48*prex_temp;
     _measurement(_index_pressure) = prex_temp;}
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet){
      double s_i;
      for (int i=0; i<_npart; i++){
      ...
      magnet /= double(_npart);
      _measurement(_index_magnet) = magnet;}
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if (_measure_cv){
      spec_heat = (tenergy_temp*_npart)*(tenergy_temp*_npart);
     // spec_heat /= double(_npart);
      _measurement(_index_cv) = spec_heat;}
...
void System :: averages(int blk){
  ...
  double coeff = _beta*_beta;

  _average     = _block_av / double(_nsteps);
  _average(_index_cv) = coeff*(_average(_index_cv)-_average(_index_tenergy)*_average(_index_tenergy)*_npart*_npart);
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy){
    coutf.open("../OUTPUT/total_energy.dat"+ to_string(_temp),ios::app);
    ...
    coutf << setw(12) << blk
          ...
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    coutf.close();
    coutf.open("../OUTPUT/total_energy(T).dat",ios::app);
    if (blk == _nblocks) {
      average  = _average(_index_tenergy);
      sum_average = _global_av(_index_tenergy);
      sum_ave2 = _global_av2(_index_tenergy);
  ...
  // MAGNET ////////////////////////////////
  if (_measure_magnet){
    coutf.open("../OUTPUT/magnetization.dat"+ to_string(_temp),ios::app);
    ...
    coutf.close();
    coutf.open("../OUTPUT/magnetization(T).dat",ios::app);
    if (blk == _nblocks) {
      average  = _average(_index_magnet);
      sum_average = _global_av(_index_magnet);
      sum_ave2 = _global_av2(_index_magnet);
  ...
  if (_measure_cv){
      ...
      //_global_av2(_index_cv) += _global_av(_index_cv)*_global_av(_index_cv);
      
      coutf.open("../OUTPUT/specific_heat.dat"+ to_string(_temp),ios::app);
      ...
      coutf << setw(12) << blk
            ...
            << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
      coutf.close();
      coutf.open("../OUTPUT/specific_heat(T).dat",ios::app);
      if (blk == _nblocks) {
      average  = _average(_index_cv);
      sum_average = _global_av(_index_cv);
      //sum_average = _beta*_beta * (_global_av(_index_tenergy2)-(_global_av(_index_tenergy)*_global_av(_index_tenergy)*double(_npart)*double(_npart)))/double(_npart);
      sum_ave2 = _global_av2(_index_cv);
      coutf << setw(12) << blk
          << setw(12) << average
          << setw(12) << sum_average/double(blk)
  ...
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){
      coutf.open("../OUTPUT/susceptibility.dat"+ to_string(_temp),ios::app);
      ...
      coutf << setw(12) << blk
            ...
            << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
      coutf.close();
      coutf.open("../OUTPUT/susceptibility(T).dat",ios::app);
    if (blk == _nblocks) {
      average  = _average(_index_chi);
      sum_average = _global_av(_index_chi);
      sum_ave2 = _global_av2(_index_chi);
...
void System :: averages_eq(int blk){

  ofstream coutf;
  double average, sum_average, sum_ave2;

  _average_eq     = _block_av / double(_n_eq_steps);
  _global_av_eq  += _average_eq;
  _global_av2_eq += _average_eq % _average_eq; // % -> element-wise multiplication

  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open("../OUTPUT/temperature_eq.dat",ios::app);
    average  = _average_eq(_index_temp);
    sum_average = _global_av_eq(_index_temp);
    sum_ave2 = _global_av2_eq(_index_temp);
    coutf << setw(12) << blk
          << setw(12) << average
          << setw(12) << sum_average/double(blk)
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
...
int System :: get_npart(){
  return _npart;
}

int System :: get_nbl_eq(){
  return _n_eq_blocks;
}

int System :: get_nsteps_eq(){
  return _n_eq_steps;
}

double System :: get_Boltzmann(){
  return 1.380649e-23;
}

void System :: set_temp(){
  _temp = _temp_restarted;
}
double System :: get_temp(){
  return _temp;
}

double System :: get_temp_equilibration(){
  return _average_eq(_index_temp);
}
