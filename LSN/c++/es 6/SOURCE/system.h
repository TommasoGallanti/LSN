...
private:
  ...
  double _J, _H, _H_MAGN;        // Parameters for the Ising Hamiltonian
  ...
  int _index_penergy, _index_kenergy, _index_tenergy, _index_tenergy2;       // Indices for accessing energy-related properties in vec _measurement
...
public: // Function declarations
  ...
  int get_npart();            // Get number particles
  ...
  void open_file_T();    // legge proprierties.dat e apre file in funzione di T che prendono solo l'average dell'ultimo blocco
  void initialize();          // Initialize system properties
  void initialize_for_iteration(double Temp);          // Initialize system properties for the iteration over temperatures
  void initialize_properties();// Initialize properties for measurement
  void finalize();            // Finalize system and clean up
  void set_temp();                // Set the temperature using T_EQ by input file
  void set_temperature(double temp);  // Set the temperature manually
  ...
  void read_configuration();  // Read system configuration from file .xyz
  void read_configuration_ising();  // Read system configuration from file .ising
