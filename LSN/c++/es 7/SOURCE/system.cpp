...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    if(_sim_type == 1){       // LJ system
      ...
      if(this->metro(i)){ //System acceptance evaluation
        _particle(i).acceptmove();
        _naccepted++;
      } else _particle(i).moveback(); //If translation is rejected, restore the old configuration
    } else {                  // Ising 1D
      if(this->metro(i)){     //System acceptance evaluation for a spin flip involving spin i
...
bool System :: metro(int i){ // System algorithm
  bool decision = false;
  ...
  if(_rnd.Rannyu() < acceptance ) decision = true; //System acceptance step
  return decision;
}
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    } else if( property == "PRINT_PERMISSION" ){
      input >> do_gofr;
      coutf << "TEMP_RESTARTED= " << _temp_restarted << endl;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = ((8.0*M_PI*_rho) / (9.0*pow(_r_cut,9)) - (8.0*M_PI*_rho)/ (3.0*pow(_r_cut,3))); // TO BE FIXED IN EXERCISE 7
        ...
        index_property++;
        _ptail = 32.0*M_PI*_rho*(1 / (9.0*pow(_r_cut,9)) - 1/ (6.0*pow(_r_cut,3))); // TO BE FIXED IN EXERCISE 7
      } else if( property == "GOFR" ){
        
        ofstream coutgr("../OUTPUT/gofr.dat");
        ofstream coutbg("../OUTPUT/gofr_block.dat");
        coutbg << "#    BLOCK:         VALUES:"  << endl;
        ...
        _gofr_bin.resize(_n_bins);   ////messo io
        _gofr_bin_averages.resize(_n_bins);   ////messo io
...
void System :: measure(){ // Measure properties
  ...
  vec distance;
  distance.resize(_ndim);
  ///vector<double> gofr_acc; // temp accumulator for gofr
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        distance(2) = this->pbc( _particle(i).getposition(2,true) - _particle(j).getposition(2,true), 2);
        dr = sqrt( dot(distance,distance) );
        max_dist = _halfside(0);
        if (dr < max_dist) {
          if (do_gofr==1){
          ///gofr_acc.push_back(dr);
          _dr_value = dr;
          _hist_gofr.push_back(dr);
          count_bin(dr);}
  ...
  if (_measure_pressure){
     prex_temp = _rho*_temp + 1.0/(3.0*_volume)*48*prex_temp +_ptail*((_npart/_volume));
     //prex_temp = _ptail*(_npart/_volume) + _rho*(2./3.)*kenergy_temp + (16./_volume) * prex_temp;
...
void System :: averages(int blk){
  ...
  if (_measure_gofr){
    if (do_gofr==1){  
      coutf.open("../OUTPUT/gofr_block.dat",ios::app);
    //  average  = _average(_index_gofr);
  //   sum_average = _global_av(_index_gofr);
  //   sum_ave2 = _global_av2(_index_gofr);
      coutf << setw(8) << blk  << "       ";
      for (int i=0; i<_gofr_bin.size(); i++){
          coutf << " "<< _gofr_bin[i] <<" ";
      }
      coutf << endl;
    //        << setw(12) << sum_average/double(blk)
...
int System::get_simtype(){
  return _sim_type;
}

void System::print_gofr(){
  ofstream coutf;
  coutf.open("../OUTPUT/gofr.dat",ios::app);
  for (int i=0; i<_gofr_bin.size(); i++){
    coutf << setw(10) << (_halfside(0)/_n_bins)*i 
          << setw(20) << _gofr_bin_averages[i]/double(_nblocks) << endl;
  }
  coutf.close();
}

void System::count_bin(double x) {
    
    ///double max_dist = sqrt( dot(_halfside,_halfside));
    double max_dist = _halfside[0];
    double delta_x = max_dist / double(_n_bins); // Larghezza di ciascun bin
    

    // Determina in quale bin cade il numero casuale
    int bin_index = std::floor(x / delta_x);

    // Incrementa di 2 il contatore del bin corrispondente
    
    double bin = delta_x*bin_index;
    double bin_inf = delta_x*bin_index;
    double bin_sup = delta_x*(bin_index+1);
    //double norm = 3/(_rho*_npart*4*M_PI*(pow(bin+x,3)-pow(bin,3)));
    double norm = 3/(_rho*_npart*4*M_PI*(pow(bin_sup,3)-pow(bin_inf,3)));
    //double norm = 3/(_rho*_npart*4*M_PI*(pow(bin+x,3)-pow(x,3)));

    _gofr_bin[bin_index] += 2*norm;
    _gofr_bin_averages[bin_index] += 2*norm/_nsteps;
}

int System::get_gofr_permission(){
  return do_gofr;
}
